...
BzDeck.controllers.Session.prototype.force_login = function () {
  ...
  this.on('LoginFormView:LoginRequested', data => {
    new BroadcastChannel('BugzillaAuthCallback').addEventListener('message', event => {
      let { email, key } = event.data;

      if (email && key) {
        this.verify_account(data.host, email, key);
      } else {
        this.trigger(':Error', { 'message': 'Your Bugzilla user name and API key could not be retrieved. \
                                             Please try again.' }); // l10n
      }
    });
  }, true);
  ...
  this.on('LoginFormView:QRCodeDecoded', data => {
    if (data.result && data.result.match(/^.+@.+\..+\|[A-Za-z0-9]{40}$/)) {
      ...
      this.trigger(':Error', { 'message': 'Your Bugzilla user name and API key could not be detected. \
                                           Please try again.' }); // l10n
    }
  }, true);
};
...
BzDeck.controllers.Session.prototype.verify_account = function (host, email, key) {
  ...
  return this.request('user', params, { 'api_key': key }).then(result => {
